...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{holdout_dashboard.png}
\caption{Holdout set diagnostics for all seven model variants. Scatter plot (top-left) shows predicted vs. actual returns; residual distribution (top-right) shows error spread per model; summary statistics table (bottom) reports median, IQR, and skewness. HMM and MLP produce tighter error distributions than baseline; Transformer is visibly dispersed.}
\label{fig:holdout}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{test_dashboard.png}
\caption{Test set dashboard for full 2022-07 to 2025-03 evaluation window. Confirms holdout ranking and shows that performance degrades gracefully outside training distribution.}
\label{fig:test}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{company_5048_overlay.png}
\caption{Company 5048 time series (representative example). Baseline CatBoost (blue), HMM correction (green), and MLP correction (orange) track actual returns (black). Corrections smooth transient overshoots without degrading fit in normal periods. Transformer (red) is too erratic, confirming instability.}
\label{fig:company_overlay}
\end{figure}

\subsection{Architecture Comparison Metrics}

\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{company_5048_mean_rmse_by_arch.png}
\caption{Mean RMSE by architecture on company 5048. Bar chart ranks all seven residual models by per-company error. HMM and MLP are clustered at low error; Transformer and Ridge show higher error; direct CatBoost residual falls in the middle.}
\label{fig:rmse_by_arch}
\end{figure}
...
\end{itemize}
\textbf{Lesson:} Sophisticated architectures require strong signals and large data. When baseline prediction is already good, residuals become harder to model, not easier.
...
\end{itemize}
\textbf{Lesson:} Two-stage models can extract regime information, but only marginal gains are possible when the baseline is strong.
...
\end{itemize}
\textbf{Lesson:} Not all residual models help. Stacking or ensembling requires that the corrector exploits a genuinely different signal source. Direct CatBoost on residuals is too similar to the baseline and adds variance without bias reduction.

\section{Implementation and Repository Structure}

The v3.1 pipeline is organized as follows:

\textbf{Preprocessing:} \texttt{preprocessing/} contains scripts for feature engineering (\texttt{build\_features.py}), CatBoost dataset construction (\texttt{build\_catboost\_dataset.py}), HMM regime labeling (\texttt{build\_hmm\_regimes.py}), and residual sequence building (\texttt{build\_residual\_sequences.py}).

\textbf{Training:} \texttt{training/} contains model training scripts: \texttt{train\_catboost.py}, \texttt{train\_hmm\_correction.py}, \texttt{train\_residual\_transformer.py}, and similar for MLP, CNN, Ridge, and Elastic Net.

\textbf{Evaluation:} \texttt{evaluation/} contains \texttt{evaluate\_holdout.py} (runs holdout metrics), \texttt{generate\_dashboards.py} (creates visualizations), and per-company diagnostic scripts.

\textbf{Pipeline entry point:} \texttt{run\_pipeline.sh} orchestrates all steps in order.

All scripts are deterministic and use fixed random seeds for reproducibility.
